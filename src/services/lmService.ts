...
class LMServiceImpl implements LMService {
   ...
   * Uses template-based generation with NLP analysis from the existing model.
  ...
  async generateSQL(nlQuery: string, schemaText: string): Promise<string> {
    ...
    try {
      logger.log('🧭 [DEBUG] Analyzing query for template-based SQL generation');

      // Use existing NLP analysis to understand the query
      const analysis = await this.analyzeQuery(nlQuery);
      logger.log('🔍 [DEBUG] Query analysis:', analysis);

      let sql = '';

      if (analysis.isProximityQuery && analysis.targetType && analysis.nearbyType) {
        // Proximity query: "Find museums with nearby French restaurants"
        // Returns both primary and nearby POIs with distance
        sql = `SELECT 
  p.id AS primary_id,
  p.name AS primary_name,
  p.type_id AS primary_type_id,
  p.latitude AS primary_latitude,
  p.longitude AS primary_longitude,
  p.address AS primary_address,
  nearby.id AS nearby_id,
  nearby.name AS nearby_name,
  nearby.type_id AS nearby_type_id,
  nearby.latitude AS nearby_latitude,
  nearby.longitude AS nearby_longitude,
  nearby.address AS nearby_address,
  (
    6371 * 2 * ASIN(SQRT(
      POWER(SIN(RADIANS(p.latitude - nearby.latitude) / 2), 2) +
      COS(RADIANS(nearby.latitude)) * COS(RADIANS(p.latitude)) *
      POWER(SIN(RADIANS(p.longitude - nearby.longitude) / 2), 2)
    ))
  ) * 0.621371 AS distance_miles
FROM pois p
CROSS JOIN pois nearby
WHERE p.type_id IN (
  SELECT pt.type_id FROM poi_types pt WHERE pt.type_name = '${analysis.targetType}'
)
AND nearby.type_id IN (
  SELECT pt2.type_id FROM poi_types pt2 WHERE pt2.type_name = '${analysis.nearbyType}'
)
AND p.id != nearby.id
AND (
  6371 * 2 * ASIN(SQRT(
...
LIMIT 500`;
      } else if (analysis.types && analysis.types.length > 0) {
        // Simple type query: "Find museums" or "Show restaurants"
        const typeConditions = analysis.types.map(t => `pt.type_name = '${t}'`).join(' OR ');
        sql = `SELECT p.id, p.name, p.type_id, p.latitude, p.longitude, p.address
FROM pois p
JOIN poi_types pt ON p.type_id = pt.type_id
WHERE ${typeConditions}
LIMIT 100`;
      } else {
        // Fallback: select all POIs
        sql = `SELECT p.id, p.name, p.type_id, p.latitude, p.longitude, p.address
FROM pois p
LIMIT 100`;
      }
      ...
      if (!validation.ok) {
        logger.log('❌ [DEBUG] Validation failed for SQL:', sql);
        const appErr = new AppError(
          `Generated SQL failed validation: ${validation.reason}`,
          ErrorCode.MODEL_INFERENCE_FAILED,
          'Generated SQL did not pass safety checks'
        );
  ...
  private validateGeneratedSQL(sql: string, schemaText: string): { ok: boolean; reason?: string } {
    ...
    if (!/^\s*(select|with)\b/i.test(sql)) return { ok: false, reason: 'Only SELECT or WITH statements allowed' };
    
    // If it starts with WITH, ensure a SELECT appears somewhere in the statement
    if (/^\s*with\b/i.test(sql) && !/\bselect\b/i.test(sql)) {
      return { ok: false, reason: 'WITH clause must contain a SELECT statement' };
    }
    ...
    // Extract table aliases (e.g., "FROM pois p" or "JOIN poi_types AS pt")
    const aliases = new Set<string>();
    const tableAliasPattern = /(?:from|join)\s+(\w+)\s+(?:as\s+)?(\w+)/gi;
    let aliasMatch;
    while ((aliasMatch = tableAliasPattern.exec(sql)) !== null) {
      if (aliasMatch[2]) {
        aliases.add(aliasMatch[2].toLowerCase());
      }
    }

    // Extract column aliases (e.g., "p.id AS primary_id" or "COUNT(*) AS total")
    const columnAliasPattern = /\sas\s+(\w+)/gi;
    let colAliasMatch;
    while ((colAliasMatch = columnAliasPattern.exec(sql)) !== null) {
      if (colAliasMatch[1]) {
        aliases.add(colAliasMatch[1].toLowerCase());
      }
    }

    // Remove string literals before tokenizing (they're not identifiers)
    const sqlWithoutStrings = sql.replace(/'[^']*'/g, '');
    const sqlTokens = sqlWithoutStrings.match(tokenRegex) || [];
    const sqlKeywords = new Set([
      'select','from','where','join','on','and','or','limit','order','by','group','having','as','distinct','inner','left','right','full','outer','cross','in','not','null','is','between','like','using','case','when','then','end','count','sum','avg','min','max','exists'
    ]);
